...
export default function ProductsTable() {
  ...
  // Buscar imagens dos produtos visíveis na página atual
  useEffect(() => {
    if (produtosFiltrados.length > 0) {
      console.log(`🖼️ Carregando imagens para ${produtosFiltrados.length} produtos`)

      // Buscar imagens em paralelo com limite de 4 requisições simultâneas
      const batchSize = 4
      const batches: any[][] = []

      for (let i = 0; i < produtosFiltrados.length; i += batchSize) {
        batches.push(produtosFiltrados.slice(i, i + batchSize))
      }
                  ...
                  <Card key={product._id || product.CODPROD} className="group hover:shadow-xl transition-all duration-300 overflow-hidden">
                    <div className="relative">
                      ...
                      >
                        ...
                        <div className={`flex flex-col items-center justify-center bg-white w-full h-full ${produtoImagens[product.CODPROD] ? 'absolute inset-0' : ''}`}>
                          <div className="text-6xl text-gray-300 font-bold">
                            {product.DESCRPROD?.charAt(0).toUpperCase() || 'P'}
                          </div>
                          {loadingImagens[product.CODPROD] && !produtoImagens[product.CODPROD] && (
                            <div className="text-[10px] text-gray-400 mt-1">Carregando...</div>
                          )}
                        </div>
                        
                        {/* Se tiver imagem, sobrepõe a inicial */}
                        {produtoImagens[product.CODPROD] && (
                          <img 
                            src={produtoImagens[product.CODPROD]} 
