...
import { Search, Filter, ShoppingCart, Plus, Minus, ChevronRight, DollarSign, Percent, Package, Boxes } from "lucide-react"
...
export function CatalogoProdutosPedido({ 
  ...
  const [produtoImagens, setProdutoImagens] = useState<{ [key: string]: string }>({})
  const [loadingImagens, setLoadingImagens] = useState<{ [key: string]: boolean }>({})
  const [unidadesSelecionadas, setUnidadesSelecionadas] = useState<{ [key: string]: string }>({})
  const [imagensCarregadas, setImagensCarregadas] = useState<{ [key: string]: boolean }>({})
  const [todasImagensCarregadas, setTodasImagensCarregadas] = useState(false)
  ...
  const buscarImagemProduto = async (codProd: string) => {
    // Verificar se já tem a imagem em cache ou está carregando
    if (produtoImagens[codProd] || loadingImagens[codProd]) {
      return produtoImagens[codProd]
    }

    // Só buscar se estiver online
    if (!navigator.onLine) {
      setImagensCarregadas(prev => ({ ...prev, [codProd]: true }))
      return null
    }

    // Marcar como carregando
    setLoadingImagens(prev => ({ ...prev, [codProd]: true }))
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setLoadingImagens(prev => ({ ...prev, [codProd]: false }))
        setImagensCarregadas(prev => ({ ...prev, [codProd]: true }))
        return null
      }
  ...
  const carregarProdutos = async () => {
    ...
    try {
      ...
      // Apenas carregar produtos sem buscar preços (isso será feito ao adicionar ao carrinho)
      ...
      // Extrair categorias únicas (usando MARCA como categoria)
  ...
  // Buscar imagens dos produtos visíveis na página atual - otimizado para carregar em paralelo
  useEffect(() => {
    setTodasImagensCarregadas(false)

    if (produtosPaginados.length > 0) {
      console.log(`🖼️ Carregando imagens para ${produtosPaginados.length} produtos da página ${paginaAtual}`)

      // Buscar imagens em paralelo com limite de 4 requisições simultâneas
      const batchSize = 4
      const batches: any[][] = []

      for (let i = 0; i < produtosPaginados.length; i += batchSize) {
        batches.push(produtosPaginados.slice(i, i + batchSize))
      }
      ...
      const loadBatches = async () => {
        for (const batch of batches) {
          await Promise.all(
            batch.map(async (produto) => {
              if (!produtoImagens[produto.CODPROD] && !loadingImagens[produto.CODPROD]) {
                await buscarImagemProduto(produto.CODPROD)
              } else if (!imagensCarregadas[produto.CODPROD]) {
                setImagensCarregadas(prev => ({ ...prev, [produto.CODPROD]: true }))
              }
            })
          )
        ...
        setTodasImagensCarregadas(true)
      }
  ...
  }, [paginaAtual, produtosPaginados])

  // Verificar se todas as imagens da página atual foram carregadas
  useEffect(() => {
    if (produtosPaginados.length === 0) {
      setTodasImagensCarregadas(true)
      return
    ...
    const todasCarregadas = produtosPaginados.every(produto => 
      imagensCarregadas[produto.CODPROD] === true
    )

    if (todasCarregadas && !todasImagensCarregadas) {
      setTodasImagensCarregadas(true)
    }
  }, [imagensCarregadas, produtosPaginados, todasImagensCarregadas])
  ...
  const handleAdicionarAoCarrinho = (produto: any) => {
    // Validar se tabela de preço está selecionada PRIMEIRO
    ...
    // Validar se unidade foi selecionada
    ...
    // Calcular valores com desconto aplicado
    // O precoCustomizado já vem ajustado pela unidade (multiplicado ou dividido)
    ...
    // Adicionar com preço customizado, desconto e unidade OBRIGATÓRIA
    const produtoComPreco = {
      ...produto,
      CODVOL: unidadeSelecionada, // Garantir que CODVOL sempre seja enviado
      preco: vlrUnitario, // Armazena o preço já ajustado pela unidade
      VLRUNIT: vlrUnitario,
      VLRTOT: vlrTotal,
      VLRDESC: vlrDesconto,
      PERCDESC: desconto,
      FATOR_UNIDADE: fatorUnidade // Armazena o fator da unidade (>1 = multiplica, <1 = divide)
    }
  ...
  const handleSelecionarUnidade = (produto: any, unidade: any) => {
    ...
    // Atualizar o estado de quantidades com o preço ajustado para esta unidade
    setQuantidades(prev => ({
      ...prev,
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        {(!todasImagensCarregadas || loading) && produtosPaginados.length > 0 && (
            ...
            return (
              <Card key={produto.CODPROD} className="group hover:shadow-xl transition-all duration-300 overflow-hidden">
                <div className="relative">
                  {/* Imagem do produto */}
                  <div 
                    className="w-full h-32 bg-white border-b flex items-center justify-center overflow-hidden relative cursor-pointer hover:opacity-80 transition-opacity"
                    onClick={() => handleVerDetalhes(produto)}
                  >
                    {/* Sempre mostra a inicial primeiro */}
                    <div className={`flex flex-col items-center justify-center bg-white w-full h-full ${produtoImagens[produto.CODPROD] ? 'absolute inset-0' : ''}`}>
                      <div className="text-6xl text-gray-300 font-bold">
                        {produto.DESCRPROD?.charAt(0).toUpperCase() || 'P'}
                      </div>
                      {loadingImagens[produto.CODPROD] && !produtoImagens[produto.CODPROD] && (
                        <div className="text-[10px] text-gray-400 mt-1">Carregando...</div>
                      )}
                    </div>
                    
                    {/* Se tiver imagem, sobrepõe a inicial */}
                    {produtoImagens[produto.CODPROD] && (
                      <img 
                        src={produtoImagens[produto.CODPROD]} 
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (preco.preco > 0) {
                                ...
                                // Se tem unidade selecionada, recalcular o preço com a unidade
